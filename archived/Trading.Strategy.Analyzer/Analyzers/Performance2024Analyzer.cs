...
using Trading.Strategy.Analyzer.Backtest.Services;
...
/// <summary>
/// 2024年性能分析器 - 专门分析2024年黄金策略表现不佳的原因
/// </summary>
public class Performance2024Analyzer : IAnalyzer
{
    private readonly IConfiguration _configuration;
    private readonly BacktestRunner _runner;
    private readonly string _dataDirectory;

    public Performance2024Analyzer(IConfiguration configuration)
    {
        _configuration = configuration;
        _runner = new BacktestRunner();
    ...
    {
        Console.WriteLine("=== 2024年策略性能分析器 ===\n");
        Console.WriteLine("目标：分析2024年黄金策略表现不佳的原因\n");

        // 从配置读取分析参数
        var analysisConfig = _configuration.GetSection("Analysis:Performance2024");
        var strategyName = analysisConfig["StrategyName"] ?? "PinBar-XAUUSD-v2";
        var startDate = DateTime.Parse(analysisConfig["StartDate"] ?? "2024-03-01");
        var endDate = DateTime.Parse(analysisConfig["EndDate"] ?? "2024-12-31");

        // 加载策略配置
        var appSettings = _configuration.Get<AppSettings>() ?? new AppSettings();
        if (!appSettings.Strategies.ContainsKey(strategyName))
        {
            Console.WriteLine($"错误：策略 '{strategyName}' 未在appsettings.json中找到");
            return;
        }

        var config = appSettings.Strategies[strategyName].ToStrategyConfig(
            strategyName,
            appSettings.Indicators.BaseEma,
            appSettings.Indicators.AtrPeriod);

        var accountSettings = new AccountSettings
        {
            InitialCapital = appSettings.Account.InitialCapital,
            Leverage = appSettings.Account.Leverage,
            MaxLossPerTradePercent = appSettings.Account.MaxLossPerTradePercent,
    ...
    private void AnalyzeLosingTrades(BacktestResult result)
    {
        Console.WriteLine("\n" + new string('=', 60));
    ...
    private void AnalyzeTimingPatterns(BacktestResult result)
    {
        Console.WriteLine("\n" + new string('=', 60));
    ...
    private void AnalyzeMarketConditions(BacktestResult result)
    {
        Console.WriteLine("\n" + new string('=', 60));

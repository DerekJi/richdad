...
Trading.AI 是一个独立的底层AI服务项目，集成了Azure OpenAI，用于增强交易信号质量。它不绑定特定策略，可以用于多种场景。
...
在 `Trading.AlertSystem.Web/appsettings.json` 中添加：
...
```json
{
  "Logging": {
    "LogLevel": {
      ...
      "Trading.AI.Services.MarketAnalysisService": "Debug"
    }
  }
}
```
